...
const Footer = () => {
    ...
    // const videoRef = useRef(null); const [isMobile, setIsMobile] = useState(false);
    ...
    <div className="w-full min-h-screen grid content-center justify-items-center overflow-hidden   bg-black ">
        {/* <div className="relative w-full max-h-1/2 md:max-h-96 border-2 border-x-0   bg-black"> */}
            
             {/* Video Background */}
            
            {/* <video
                ref={videoRef}
                className={`absolute top-0 left-0 w-full h-full md:max-h-96 object-cover opacity-50 grayscale transition-all duration-900 ease-in-out md:${isHovered ? 'opacity-50' : 'opacity-100'}`}
                muted
                loop
                onMouseEnter={handleMouseEnter}
                onMouseLeave={handleMouseLeave}
            >
                <source src={assets.Footer} type="video/mp4" />
                Your browser does not support the video tag.
            </video> */}
           
            {/* Main Content */}
             {/* <div className="relative w-full max-w-screen-lg m-6 text-white grid md:block "
                onMouseEnter={handleMouseEnter}
                onMouseLeave={handleMouseLeave}>
                 */}
                {/* Heading at Top-Left */}
                {/* <h1 className="text-7xl bg-transparent   font-bold m-6 md:m-10">
                    How can we help you.
                    <br />
                    Get in touch
                </h1> */}
                ...
                {/* <p className="text-lg mt-4 ml-6 md:ml-10 md:w-2/3">
            ...
            {/* <div className="md:absolute py-5 pl-44 md:px-0 md:py-0 md:bottom-6 md:right-6">
                    <a className="bg-blue-600 text-white poppins-medium text-center rounded-lg shadow px-10 py-3 inline-block">
                    Get Connected <LiaConnectdevelop className='inline-block ml-2 animate-spin size-6'/>
                    </a>
                </div>
            </div>
        </div>  */}
     <div className="md:w-2/3 w-full px-4 text-white flex  flex-col">
            ...
            <div className="flex flex-col">
                ...
                <p className="w-full text-center my-12 text-gray-600">Copyright © 2025 Anant0551.dev Creative</p>
            </div>
        </div>
    </div>
    )
